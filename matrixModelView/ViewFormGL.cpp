...
{
    setModelMatrix(model->getModelX(), model->getModelY(), model->getModelZ(), model->getModelAngleX(), model->getModelAngleY(), model->getModelAngleZ());
}
...
void ViewFormGL::check_Box(int id) {
    switch (id) {
    case IDC_CHECK4:
        CheckBox_Object_X.check();
        break;
    case IDC_CHECK5:
        CheckBox_Object_Y.check();
        break;
    case IDC_CHECK6:
        CheckBox_Object_Z.check();
        break;
    }
}

void ViewFormGL::uncheck_Box(int id) {
    switch (id) {
    case IDC_CHECK4:
        CheckBox_Object_X.uncheck();
        break;
    case IDC_CHECK5:
        CheckBox_Object_Y.uncheck();
        break;
    case IDC_CHECK6:
        CheckBox_Object_Z.uncheck();
        break;
    }
}

void ViewFormGL::clearObjectRotation() {
    CheckBox_Object_X.uncheck();
    CheckBox_Object_Y.uncheck();
    CheckBox_Object_Z.uncheck();
}

bool ViewFormGL::boxRotationOXIsCheck() {
    return CheckBox_Object_X.isChecked();
}

bool ViewFormGL::boxRotationOYIsCheck() {
    return CheckBox_Object_Y.isChecked();
}

bool ViewFormGL::boxRotationOZIsCheck() {
    return CheckBox_Object_Z.isChecked();
}

void ViewFormGL::uncheckObject() {
    Radio_Object_Teapot.uncheck();
    Radio_Object_Cube.uncheck();
    Radio_Object_Torus.uncheck();
    Radio_Object_Sphere.uncheck();
    Radio_OBject_Cylinder.uncheck();
    Radio_Object_Wheel.uncheck();
    Radio_Object_Cone.uncheck();
}

void ViewFormGL::setViewObject(int id_obj){
    uncheckObject();
    switch (id_obj) {
    case IDC_RADIO1:
        Radio_Object_Teapot.check();
        break;
    case IDC_RADIO2:
        Radio_Object_Cube.check();
        break;
    case IDC_RADIO3:
        Radio_Object_Torus.check();
        break;
    case IDC_RADIO4:
        Radio_Object_Sphere.check();
        break;
    case IDC_RADIO5:
        Radio_OBject_Cylinder.check();
        break;
    case IDC_RADIO9:
        Radio_Object_Wheel.check();
        break;
    case IDC_RADIO10:
        Radio_Object_Cone.check();
        break;
    }
}

void ViewFormGL::setViewShape(int id_shape) {
    switch (id_shape) {
    case IDC_RADIO6: // Point
        radio_Shape_Point.check();
        radio_Shape_Line.uncheck();
        radio_Shape_Face.uncheck();
        Radio_Texture.uncheck();
        break;
    case IDC_RADIO7: // Line
        radio_Shape_Point.uncheck();
        radio_Shape_Line.check();
        radio_Shape_Face.uncheck();
        Radio_Texture.uncheck();
        break;
...
void ViewFormGL::setViewFog() {
    if (!radio_Fog.isChecked()) radio_Fog.check(); else radio_Fog.uncheck();
}
...
void ViewFormGL::initControls(HWND handle)
{
    ...
    Radio_Object_Teapot.set(handle, IDC_RADIO1);
    Radio_Object_Cube.set(handle, IDC_RADIO2);
    Radio_Object_Torus.set(handle, IDC_RADIO3);
    Radio_Object_Sphere.set(handle, IDC_RADIO4);
    Radio_OBject_Cylinder.set(handle, IDC_RADIO5);
    Radio_Object_Wheel.set(handle, IDC_RADIO9);
    Radio_Object_Cone.set(handle, IDC_RADIO10);

    CheckBox_Object_X.set(handle, IDC_CHECK4);
    CheckBox_Object_Y.set(handle, IDC_CHECK5);
    CheckBox_Object_Z.set(handle, IDC_CHECK6);

    radio_Shape_Point.set(handle, IDC_RADIO6);
    radio_Shape_Line.set(handle, IDC_RADIO7);
    radio_Shape_Face.set(handle, IDC_RADIO8);
    Radio_Texture.set(handle, IDC_RADIO11);
    radio_Fog.set(handle, IDC_RADIO12);
...
void ViewFormGL::updateMatrices()
{
    ...
    wss << L"glRotatef(" << model->getCameraAngleX() << L",0,0,1);\n"
        << L"glRotatef(" << -model->getCameraAngleY() << L",0,1,0);\n"
        << L"glRotatef(" << model->getCameraAngleZ() << L",1,0,0);\n"

...
LRESULT CALLBACK Win::windowProcedure(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEWHEEL:
        returnValue = ctrl->mouseWheel((short)LOWORD(wParam), (short)HIWORD(wParam), (short)LOWORD(lParam), (short)HIWORD(lParam));   // state, delta, x, y
        break;
    ...
    //case WM_DPICHANGED:
    //    returnValue = ctrl->dpiChanged(LOWORD(wParam), HIWORD(wParam), (RECT*)lParam);  // x-dpi, y-dpi, rect
...
INT_PTR CALLBACK Win::dialogProcedure(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
        ctrl->size(LOWORD(lParam), HIWORD(lParam), (int)wParam);                // width, height, type
        return true;
